...
import { Decision, Evidence, MetricKey, Risk } from "@/lib/types";
...
function CityLab({s,update,district,setDistrict,layer,setLayer,open}:{s:State;update:(p:Partial<State>)=>void;district:any;setDistrict:(d:any)=>void;layer:any;setLayer:(l:any)=>void;open:(e:Evidence)=>void}){
 const layers=["heat","water","carbon","flood","assets"] as const;
 const select=(d:any)=>{setDistrict(d);update({explored:[...new Set([...s.explored,d.id])]})};
 return <>
